...
check:
	$(DOCKER) build -t zeekjs-check -f ./docker/fedora-42.Dockerfile .

check-nightly:
	$(DOCKER) build --build-arg STAMP=$(shell date +%Y-%m-%d) -t zeekjs-check-nightly -f ./docker/fedora-42-nightly.Dockerfile .
